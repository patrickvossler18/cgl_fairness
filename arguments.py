...
def get_args():
    ...
    parser.add_argument(
        "--term", default=20, type=int, help="the period for recording train acc"
    )
    parser.add_argument(
        "--record", default=False, action="store_true", help="record using tensorboardX"
    )
    parser.add_argument(
        "--result-dir",
        default="./results/",
    ...
    parser.add_argument(
        "--log-dir", default="./logs/", help="directory to save logs (default: ./logs/)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--img-size", default=224, type=int, help="img size for preprocessing"
    )
    parser.add_argument(
        "--num-workers",
        default=2,
        type=int,
        help="the number of thread used in dataloader",
    )
    parser.add_argument(
        "--labelwise",
        default=False,
        action="store_true",
        help="balanced sampling over groups",
    )
    # only for celebA
    parser.add_argument(
        "--target", default="Attractive", type=str, help="target attribute for celeba"
    )
    parser.add_argument(
        "--add-attr", default=None, help="additional group attribute for celeba"
    )
    ...
    parser.add_argument(
        ...
        default="mlp",
        required=True,
        choices=["mlp", "resnet18", "resnet18_dropout"],
    )
    ...
    parser.add_argument(
        "--t-device", default=0, type=int, help="teacher cuda device number"
    )
    # optimization
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        "--teacher-type", default=None, choices=["mlp", "resnet18", "resnet18_dropout"]
    )
    parser.add_argument("--teacher-path", default=None, help="teacher model path")

    # For reweighting & adv
    parser.add_argument(
        "--reweighting-target-criterion",
        default="eo",
        type=str,
        help="fairness criterion",
    )
    parser.add_argument(
        "--iteration", default=10, type=int, help="iteration for reweighting"
    )
    parser.add_argument(
        "--ups-iter", default=10, type=int, help="iteration for reweighting"
    )
    parser.add_argument(
        "--eta",
        default=0.001,
        type=float,
        help="adversary training learning rate or lr for reweighting",
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=float,
        help="the ratio of group annotation for a training set",
    )
    parser.add_argument(
        "--version",
        default="",
        type=str,
        help="version about how the unsupervised data is used",
    )
