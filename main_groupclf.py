...
def measure_ood(ind_probs, ood_probs, tpr_thres=0.95):
    ...
    labels = np.append(np.ones_like(ind_probs) * 1, np.ones_like(ood_probs) * 2)
    ...
    if n_neg != n_pos:
        print(
            f"warning: n_neg ({n_neg}) != n_pos ({n_pos}). It may shows weird detection acc"
        )
        print(
            f"current threshold ({thresholds[temp_idx]}): FPR ({fpr[temp_idx]}), TPR ({tpr[temp_idx]})"
        )
...
def mc_dropout(model, inputs):
    ...
    for _ in range(f_pass):
        ...
        out_prob_nl.append(
            F.softmax(outputs / temp_nl, dim=1)
        )  # for selecting negative pseudo-labels
...
def main():
    ...
    if args.modelpath is not None:
        ...
        model.fc = nn.Linear(
            in_features=model.fc.weight.shape[1], out_features=num_groups, bias=True
        )
    ...
    trainer_ = trainer.TrainerFactory.get_trainer(
        args.method, model=model, args=args, optimizer=optimizer, teacher=teacher
    )
    ...
    if args.mode == "train":
        ...
        print(
            "Training Time : {} hours {} minutes".format(
                int(train_t / 60), (train_t % 60)
            )
        )
        ...
        if args.version == "groupclf_val":
            opt_thres = predict_thres(
                results["probs"], results["true_idxs"], results["false_idxs"], val_idxs
            )
    ...
    if args.evalset == "all":
        trainer_.compute_confusion_matix(
            "train", train_loader.dataset.num_classes, train_loader, log_dir, log_name
        )
        trainer_.compute_confusion_matix(
            "test", test_loader.dataset.num_classes, test_loader, log_dir, log_name
        )
